...
class getSetData {
    ...
    public function setData() {
        try {
            $db = new DbConnector();
            $db->connect();
            $query = "UPDATE customer SET fName = ?, lName = ?, email = ?, contact = ?, img_path = ? WHERE CustomerId = ?";
    ...
    public function setManagerData() {
        try {
            $db = new DbConnector();
            $db->connect();
            $query = "UPDATE manager SET fName = ?, lName = ?, email = ?, contact = ?, img_path = ? WHERE user_id = ?";
            $stmt = $db->preQury($query);
            $stmt->bindValue(1, $this->fName);
            $stmt->bindValue(2, $this->lName);
            $stmt->bindValue(3, $this->email);
    ...
    public function getManagerName() {
        try {
            $db = new DbConnector();
            $db->connect();
            $query = "SELECT fName FROM manager WHERE user_id = ?";
            $stmt = $db->preQury($query);
            $stmt->bindValue(1,  $this->user_id);
            $stmt->execute();
            $data = $stmt->fetch(PDO::FETCH_ASSOC);
